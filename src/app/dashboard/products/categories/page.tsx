...
export default async function CategoriesPage({ searchParams }: CategoriesPageProps) {
  ...
  const searchCondition = searchParams.search
    ? Prisma.sql`(
        LOWER(c.name) LIKE LOWER(${`%${searchParams.search}%`}) OR
        LOWER(c.description) LIKE LOWER(${`%${searchParams.search}%`})
      )`
    : Prisma.sql`TRUE`;

  // Fetch categories with related data
  const [categories, total] = await Promise.all([
    prisma.$queryRaw<CategoryWithRelations[]>`
      SELECT 
        c.id,
        c.name,
        c.description,
        p.name as "parent.name",
        (SELECT COUNT(*) FROM "Product" WHERE "categoryId" = c.id) as "products_count",
        (SELECT COUNT(*) FROM "Category" WHERE "parentId" = c.id) as "children_count"
      FROM "Category" c
      LEFT JOIN "Category" p ON c."parentId" = p.id
      WHERE ${searchCondition}
      ORDER BY 
        CASE 
          WHEN ${sortBy} = 'name' AND ${sortOrder} = 'asc' THEN c.name
          WHEN ${sortBy} = 'name' AND ${sortOrder} = 'desc' THEN c.name
          WHEN ${sortBy} = 'createdAt' AND ${sortOrder} = 'asc' THEN c."createdAt"::text
          WHEN ${sortBy} = 'createdAt' AND ${sortOrder} = 'desc' THEN c."createdAt"::text
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            {Number(total[0].count)} categories found
          </p>
        </div>
          ...
          <tbody className="divide-y divide-gray-200 bg-white">
            {categories.map((category) => (
              <tr key={category.id}>
                ...
                <td className="whitespace-nowrap px-6 py-4">
                  <div className="text-sm text-gray-900">{category.parent?.name || '-'}</div>
                </td>
                <td className="whitespace-nowrap px-6 py-4">
                  <div className="text-sm text-gray-900">{category.products_count}</div>
                </td>
                <td className="whitespace-nowrap px-6 py-4">
                  <div className="text-sm text-gray-900">{category.children_count}</div>
                </td>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between border-t border-gray-200 bg-white px-4 py-3 sm:px-6">
          <div className="flex flex-1 justify-between sm:hidden">
            <Link
              href={`/dashboard/products/categories?page=${page - 1}`}
